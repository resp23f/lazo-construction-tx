...
export default function AboutContent() {
  ...
  return (
    <>
        ...
        <Container>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-12 lg:gap-16 items-center">
            ...
            <ScrollReveal delay={100}>
              <h2 className="font-heading text-xl sm:text-3xl font-bold text-primary mb-4 sm:mb-6">
                {t("aboutPage.storyHeading")}
              </h2>
              <div className="prose prose-sm sm:prose-lg text-text-muted space-y-3 sm:space-y-5">
      ...
      <section className="py-10 sm:py-16 bg-gradient-to-b from-surface to-background">
        <Container>
            ...
            {coreValues.map((value, index) => {
              ...
              return (
                <ScrollReveal key={value.titleKey} delay={index * 100}>
                  <div className="bg-surface p-4 sm:p-6 rounded-xl border border-gray-100 h-full">
                    <div className="inline-flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-primary mb-3 sm:mb-4">
                      <Icon className="h-5 w-5 sm:h-6 sm:w-6 text-white" strokeWidth={1.5} />
                    </div>
                    <h3 className="font-heading font-bold text-sm sm:text-lg text-text">{t(value.titleKey)}</h3>
                    <p className="mt-1 sm:mt-2 text-text-muted text-xs sm:text-sm">{t(value.descKey)}</p>
                  </div>
                </ScrollReveal>
              );
            })}
